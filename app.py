...
@st.cache_data(ttl=300, show_spinner=False)
def load_google_campaigns(customer_id, access_token, gaql_date_range):
    query = f"""
        SELECT
            campaign.id,
            campaign.name,
...
def parse_google_campaign_rows(results):
    rows = []
    for r in results:
...
def parse_google_ad_group_rows(results):
    rows = []
    for r in results:
        campaign = r.get("campaign", {})
        ...
        rows.append({
            "Кампанія": campaign.get("name", ""),
            "Група оголошень": ad_group.get("name", ""),
            "Статус": ad_group.get("status", ""),
            "Покази": impressions,
            "Кліки": clicks,
            "Витрати (zł)": round(cost, 2),
            "CTR (%)": round(ctr, 2),
...
# Shared date options (Meta uses date_preset strings, Google uses GAQL date range names)
DATE_OPTIONS = {
...
with st.sidebar:
    ...
    if ACCESS_TOKEN:
        try:
            ...
            st.warning(f"Meta: не вдалося завантажити акаунти: {e}")
    else:
        st.warning("META_ACCESS_TOKEN не знайдено")
    ...
    st.caption("Дані оновлюються при зміні параметрів")
...
with main_tab_meta:
    if not ACCESS_TOKEN:
        ...
        meta_tab_campaigns, meta_tab_adsets, meta_tab_ads = st.tabs(
            ["📁 Кампанії", "🗂 Групи оголошень", "🎯 Оголошення"]
        )

        # --- META: CAMPAIGNS ---
        with meta_tab_campaigns:
            with st.spinner("Завантаження кампаній..."):
                try:
            ...
            if insights or campaigns:
                ...
                active_only = st.checkbox("Тільки активні кампанії", value=True, key="meta_active_campaigns")
        ...
        # --- META: AD SETS ---
        with meta_tab_adsets:
            with st.spinner("Завантаження груп оголошень..."):
                try:
            ...
            if not df2.empty:
                campaign_filter = st.multiselect(
                    "Фільтр по кампанії", df2["Кампанія"].unique(), key="meta_adset_campaign_filter"
                )
                if campaign_filter:
        ...
        # --- META: ADS ---
        with meta_tab_ads:
            with st.spinner("Завантаження оголошень..."):
                try:
            ...
            if not df3.empty:
                camp_filter = st.multiselect(
                    "Фільтр по кампанії", df3["Кампанія"].unique(), key="meta_ad_camp_filter"
                )
                if camp_filter:
...
with main_tab_google:
    ...
    required_google_secrets = ["GOOGLE_DEVELOPER_TOKEN", "GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "GOOGLE_REFRESH_TOKEN"]
    missing_secrets = [s for s in required_google_secrets if not st.secrets.get(s)]

    if missing_secrets:
        st.error(f"Відсутні секрети для Google Ads: {', '.join(missing_secrets)}")
    else:
        # Get access token once for both sub-tabs
        google_access_token = None
        google_token_error = None
        try:
            google_access_token = google_get_access_token()
        except Exception as e:
            google_token_error = str(e)
        ...
        if google_token_error:
            ...
            # --- GOOGLE: CAMPAIGNS ---
            with google_tab_campaigns:
                with st.spinner(f"Завантаження кампаній Google Ads ({google_account_label})..."):
                    try:
                        g_campaign_results = load_google_campaigns(
                            google_customer_id, google_access_token, google_date_range
                        )
                        g_campaign_rows = parse_google_campaign_rows(g_campaign_results)
                    except Exception as e:
                        st.error(f"Помилка завантаження кампаній Google: {e}")
                        g_campaign_rows = []

                if g_campaign_rows:
                    df_gc = pd.DataFrame(g_campaign_rows)
                    ...
                    gc_active_only = st.checkbox("Тільки активні кампанії", value=True, key="google_active_campaigns")
                    if gc_active_only and not df_gc.empty and "Статус" in df_gc.columns:
            ...
            # --- GOOGLE: AD GROUPS ---
            with google_tab_adgroups:
                with st.spinner(f"Завантаження груп оголошень Google Ads ({google_account_label})..."):
                    try:
                        g_adgroup_results = load_google_ad_groups(
                            google_customer_id, google_access_token, google_date_range
                        )
                        g_adgroup_rows = parse_google_ad_group_rows(g_adgroup_results)
                    except Exception as e:
                        st.error(f"Помилка завантаження груп оголошень Google: {e}")
                        g_adgroup_rows = []

                if g_adgroup_rows:
                    df_gag = pd.DataFrame(g_adgroup_rows)

                    if not df_gag.empty:
                        gc_camp_filter = st.multiselect(
                            "Фільтр по кампанії", df_gag["Кампанія"].unique(), key="google_adgroup_campaign_filter"
                        )
                        if gc_camp_filter:
                            df_gag = df_gag[df_gag["Кампанія"].isin(gc_camp_filter)]
